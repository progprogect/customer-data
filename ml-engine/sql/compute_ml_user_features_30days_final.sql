-- =========================
-- ML User Features 30 Days Computation - Final Version
-- =========================
...
freq_90_2 AS (
  SELECT 
    ...
    dr.snapshot_date,
    COUNT(*) AS cnt
  FROM orders o, params2 p, date_range2 dr
  WHERE o.status = ANY(p.ok_status)
    AND o.created_at >= (dr.snapshot_date - (p.win_f || ' days')::interval)
    AND o.created_at <  (dr.snapshot_date + '1 day'::interval)
  GROUP BY o.user_id, dr.snapshot_date
...
cats_180_2 AS (
  ...
  FROM orders o
  JOIN order_items oi ON oi.order_id = o.order_id
  JOIN products pr    ON pr.product_id = oi.product_id, params2 p, date_range2 dr
  WHERE o.status = ANY(p.ok_status)
    AND o.created_at >= (dr.snapshot_date - (p.win_m || ' days')::interval)
    AND o.created_at <  (dr.snapshot_date + '1 day'::interval)
  GROUP BY o.user_id, dr.snapshot_date
),
u2 AS (SELECT user_id FROM users),
feat_all2 AS (
  SELECT
    u.user_id,
    dr.snapshot_date,
    ...
    COALESCE(c.cats, 0)                              AS categories_unique
  FROM u2 u
  CROSS JOIN date_range2 dr
  LEFT JOIN last_order2 lo ON lo.user_id = u.user_id
  LEFT JOIN freq_90_2    f  ON f.user_id = u.user_id AND f.snapshot_date = dr.snapshot_date
  LEFT JOIN mon_180_2    m  ON m.user_id = u.user_id AND m.snapshot_date = dr.snapshot_date
  LEFT JOIN life_all2   l  ON l.user_id = u.user_id
  LEFT JOIN cats_180_2   c  ON c.user_id = u.user_id AND c.snapshot_date = dr.snapshot_date
),
buyers2 AS (
  SELECT DISTINCT user_id FROM life_all2 WHERE orders_lt > 0
)
