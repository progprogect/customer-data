-- =========================
-- Анализ данных для ценовой эластичности
...
SELECT 
    ...
    AVG(unique_prices_in_month) as avg_price_changes_per_month,
...
SELECT 
    ...
    AVG(oi.unit_price) as avg_historical_price,
...
ORDER BY ABS(current_vs_avg_diff_pct) DESC
LIMIT 20;
...
WITH price_quality_metrics AS (
    SELECT 
        ...
        (SELECT COUNT(DISTINCT product_id) FROM order_items GROUP BY product_id HAVING COUNT(DISTINCT unit_price) > 1) as products_with_price_variations,
        
        -- Метрики вариаций
        (SELECT AVG(price_points) FROM (
...
SELECT 
    ...
    ROUND(avg_price_points, 2) as avg_price_points,
    data_span_days,
    ROUND(avg_price_variance_pct, 2) as avg_price_variance_pct,
