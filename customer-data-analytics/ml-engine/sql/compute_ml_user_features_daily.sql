-- =========================
-- ML User Features Daily Computation
-- =========================
-- Ежедневный пересчёт RFM-признаков для всех пользователей
-- Создаёт 30 снапшотов за последние 30 дней

-- ========= ПАРАМЕТРЫ ОКОН =========
WITH params AS (
  SELECT
    CURRENT_DATE::date           AS snap,     -- дата среза
...
freq_90 AS (
  ...
  FROM orders o, params p
  WHERE o.status = ANY(p.ok_status)
    AND o.created_at >= (p.snap - (p.win_f || ' days')::interval)
    AND o.created_at <  (p.snap + '1 day'::interval)
  GROUP BY o.user_id
),
mon_180 AS (
  SELECT o.user_id,
  ...
  FROM orders o, params p
  WHERE o.status = ANY(p.ok_status)
    AND o.created_at >= (p.snap - (p.win_m || ' days')::interval)
    AND o.created_at <  (p.snap + '1 day'::interval)
  GROUP BY o.user_id
),
life_all AS (
...
cats_180 AS (
  ...
  JOIN products pr    ON pr.product_id = oi.product_id, params p
  WHERE o.status = ANY(p.ok_status)
    AND o.created_at >= (p.snap - (p.win_m || ' days')::interval)
    AND o.created_at <  (p.snap + '1 day'::interval)
  GROUP BY o.user_id
...
feat_all AS (
  SELECT
    u.user_id,
    p.snap                                           AS snapshot_date,
    CASE WHEN lo.last_dt IS NULL
         THEN NULL
         ELSE GREATEST(0, (p.snap - lo.last_dt))::int
    END                                              AS recency_days,
  ...
  LEFT JOIN freq_90    f  ON f.user_id  = u.user_id
  LEFT JOIN mon_180    m  ON m.user_id  = u.user_id
  LEFT JOIN life_all   l  ON l.user_id  = u.user_id
  LEFT JOIN cats_180   c  ON c.user_id  = u.user_id
),
